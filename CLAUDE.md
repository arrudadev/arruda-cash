...
- Services follow AdonisJS's own convention (`node ace make:service <name>`): a **named-export class**, decorated `@inject()`, holding its own dependencies (other services, framework services like `Logger`/`Disk`) as `protected` constructor params — not a plain object of functions. See `app/services/transaction_service.ts` (depends on `CategoryService`) for the pattern; `app/services/auth_service.ts` for one with no dependencies.
- Controllers that use a service also get `@inject()` plus a constructor param for it (see any controller in `app/controllers/`) — AdonisJS's router always constructs controllers through the IoC container, so this "just works" without touching `start/routes.ts`.
- **Gotcha:** when a service is imported into another file *only* to type a constructor param (e.g. `constructor(protected categoryService: CategoryService)`), Biome's `useImportType` rule wants to rewrite that import to `import type { ... }`. Don't let it — `@inject()`'s reflection metadata needs the real (value) import at runtime to resolve the class, and `import type` erases it, silently breaking DI at boot. Every such import in this codebase carries a `// biome-ignore lint/style/useImportType: needed at runtime for @inject()'s reflection metadata` comment for this reason — copy the same comment on any new one.
- In tests, instantiate services directly — `new CategoryService()`, `new TransactionService(new CategoryService())` — no container needed for unit tests since there's nothing but plain constructor args to satisfy. Functional/browser tests go through real HTTP requests, so the container wires everything up exactly as it does for `node ace serve`.
