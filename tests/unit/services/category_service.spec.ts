...
test.group('CategoryService', (group) => {
  ...
  test('listForUser only returns categories owned by that user, sorted by name', async ({
    ...
    await CategoryService.create(owner.id, { name: 'Transport', color: '#3b82f6', type: 'expense' })
    await CategoryService.create(owner.id, { name: 'Groceries', color: '#22c55e', type: 'expense' })
